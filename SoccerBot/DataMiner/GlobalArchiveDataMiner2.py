...
def findBest(dataFrame, winnerColumns, looserColumns):
    ...
    for k, v in idx.iteritems():
        element = """GameStrategy(formation = '%s', strategy = '%s', tactic = '%s' ) : '%s'"""
        formatted = element % (  k[0], k[1], k[2], v[0])
...
"""
    ...
    for r in z2.index.get_values():
        ...
        if bs == None:
            continue
        element = """GameStrategy(formation = '%s', strategy = '%s', tactic = '%s' ) : GameStrategy(formation = '%s', strategy = '%s', tactic = '%s' )"""
        formatted = element % (  r[0], r[1], r[2], bs[0], bs[1], bs[2])
